...
def _(np, p_si_given_sj, plt, sx):
    ...
    ax_profile.set_ylabel("$d_x$", rotation=0, labelpad=10)
    ax_profile.yaxis.set_label_position("right")
    _fig.suptitle("A", x=0)
    ax_profile.set_title("stimulus", loc="left", x=0.2)
    ax_profile.set_xlabel("$p(d^{CTX}_x|d_j)$", fontsize=12)
    ax_profile.annotate('',
                        xy=(1.2, 1),
                        xytext=(1.2, -1),
...
def _(np, plt):
    ...
    _ax_profile.set_ylabel(r"$\mathbf{\ast_t}$", rotation=0, labelpad=10, fontsize=18)
    _ax_profile.set_xlabel(r"$t$", loc="right", fontsize=12)
