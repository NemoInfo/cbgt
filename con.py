...
def _(np):
    def w_cluster(stn_count, gpe_count, gpi_count):
      ...
      _base[[2, -2]] = 1
      stn_w_g_s = np.vstack([np.roll(_base, i) for i in range(gpe_count)])
      return {"gpe_w_g_g": gpe_w_g_g, "gpe_w_s_g": gpe_w_s_g, "stn_w_g_s": stn_w_g_s}
    ...
    def w_n_net(stn_count, gpe_count, gpi_count):
      ...
      stn_w_g_s = np.zeros((gpe_count, stn_count))
      stn_w_g_s[np.arange(1, gpe_count, 3), np.arange(stn_count)] = 1
...
def _(
    ...
    np.random.seed(69)
    stn_count, gpe_count, gpi_count = 10, 30
...
def _(df, df2, plt):
    ...
    plt.imshow(df["stn"]["w_g_s"][0], cmap='gray', interpolation='nearest')
    plt.colorbar(label='Connection Strength')
    plt.ylabel("GPe")
    plt.xlabel("STN")
    plt.tight_layout()
    ...
    plt.imshow(df2["stn"]["w_g_s"][0], cmap='gray', interpolation='nearest')
    plt.colorbar(label='Connection Strength')
    plt.ylabel("GPe")
    plt.xlabel("STN")
    plt.tight_layout()
