...
def _(enforce_min_isi, np, p_si_given_sj, plt):
    ...
    _fig, _axs = plt.subplots(N, 1, figsize=(10, 1 * N), sharex=True)
    for _i, (_ax, spike) in enumerate(zip(_axs, spikes.T)):
      _ax.vlines(ts[spike], 0, 1, color='black') # vertical lines where spikes occur
      _ax.fill_between(ts, 0, 1, where=stims[:, _i] > 0, color="gray", alpha=0.3)
      _ax.yaxis.set_label_position("right")
      _ax.set_xlim((ts[0], ts[-1]))
      _ax.set_yticks([])
      _ax.set_ylabel(f"CTX {_i+1}", rotation=0, labelpad=30)

    _axs[0].set_title('Poisson Spike Train')
    _axs[-1].set_xticks(np.arange(0, 4001, 1000))
...
def _(np, p_si_given_sj, plt, sx):
    _fig = plt.figure(figsize=(2, 10))
    ax_profile = _fig.gca()
    fspos = np.linspace(0, 1, 100)
    ax_profile.set_ylim(0, 1)
    ax_profile.set_xlim(-0.1, 1.2)
    ax_profile.set_yticks([])
    ax_profile.set_xticks([])
    ax_profile.plot(1 - p_si_given_sj(fspos, sx), fspos, 'k')
...
@app.cell
def _(N, convolve_spike_train, dt, np, plt, rayleigh_kernel, spikes, ts):
    sigma = 5.0
    t_max = 50.0
    kernel = rayleigh_kernel(sigma, dt, t_max)
    syns = convolve_spike_train(spikes, kernel)

    fig, axs = plt.subplots(N, 1, figsize=(10, 1 * N), sharex=True, sharey=True)
    ...
    axs[0].set_title("Cortical synaptic output")
    axs[-1].set_xticks(np.arange(0, 4001, 1000))
    axs[-1].set_xlabel('Time (ms)')

    plt.savefig("poisson3.svg")
    plt.show()
    return ax, axs, fig, i, kernel, sigma, syn, syns, t_max
